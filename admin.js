/** admin.js — Panel Admin (CRUD Pegawai + Reset Password + Password Sementara) */
...
async function ensureAdmin(){
  try {
    ...
    // Cari profil (prioritas employees)
    let profile = null;
...
async function loadStats(){
  try {
    ...
    // Absensi hari ini (butuh RLS khusus untuk rekap semua pegawai)
    let todayCount = 0;
    {
      const d = new Date();
...
function openEmployeeModal(mode='create', rec=null){
  ...
  // Field password sementara (hanya dipakai saat create)
  const tempPw = qs('empTempPassword');
  ...
  if (mode === 'create') {
    ...
    if (tempPw) tempPw.value = ''; // di edit, abaikan/biarkan kosong
  }
...
async function saveEmployee(e){
  ...
  try {
    if (id) {
      ...
      // === MODE CREATE ===
      // 1) minta server buat user Auth (email + password sementara + must_change_password=true)
      const empTempPassword = (qs('empTempPassword')?.value || '').trim();

      const { data: { session } } = await window.supabaseClient.auth.getSession();
      const accessToken = session?.access_token;
      if (!accessToken) {
        errBox.textContent = 'Sesi admin tidak valid. Silakan login ulang.';
        errBox.classList.remove('hidden');
        return;
      }

      const resp = await fetch('/.netlify/functions/create-employee-user', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          email,
          tempPassword: empTempPassword,                 // boleh kosong → server auto-generate
          employeePayload: { organization_id: ADMIN_ORG_ID }
        })
      });

      const json = await resp.json().catch(() => ({}));
      if (!resp.ok) {
        throw new Error(json?.error || `Gagal membuat user auth (status ${resp.status})`);
      }

      const auth_user_id   = json.auth_user_id;
      const temp_password  = json.temp_password; // tampilkan sekali untuk dicatat admin

      // 2) Insert row employees (link ke auth_user_id)
      const payload = { name, nik, email, position, organization_id: ADMIN_ORG_ID, user_id: auth_user_id };
      const { error } = await window.supabaseClient.from('employees').insert(payload);
      if (error) throw error;

      // 3) Tampilkan password sementara (sekali)
      if (temp_password) {
        alert(
          "Akun berhasil dibuat.\n\n" +
          `Email: ${email}\n` +
          `Password sementara: ${temp_password}\n\n` +
          "Catat password ini dan sampaikan ke pegawai (secara privat). " +
          "Pegawai WAJIB mengganti password saat pertama login."
        );
      }
    }
...
/* ======== Reset Password (email recovery via server function) — opsional, tetap dipertahankan ======== */
